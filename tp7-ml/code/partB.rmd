...
```{r}
  ...
  #test the model
  preds_tree <- predict(rf, dataTest)
  
  #generate result
  result <- data.frame(id = dataTest$id, preds_tree)
  ...
  #confusionMatrix(as.factor(result$inclinacion_peligrosa), as.factor(data_validation$inclinacion_peligrosa))
